...
function Scraper(source){
	...
	this.history = 25; //articles to save in db
	this.success = [];
}
		...
		async.mapSeries( 
			...
			function(item, callback){
				...
				request(item, function(err, res, body){
					...
					//add to last batch
					fetch.push(item);
					//save image to facebook
					if(result.images.length == 0){
							client.lpush('articles:' + self.label, JSON.stringify(result), redis.print);
							client.hset(self.label, result.url, result.article, redis.print);
...
AdduOnline.prototype.scrapeImage = function(dom){
	...
	if(img.length == 0){
		img = dom.find('.slides_control img:first');
	}
	img = img.attr('src');
	return img;
}
