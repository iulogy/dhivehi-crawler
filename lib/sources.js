...
fb.setAccessToken("AAAFL06l6XZBABAELYe3XpYwcIdy719x0DzRvZCnyDoTrYs0GbCblYIZAAtuDbKcUT0efZCLKui8FsKcrR4EKlZBIDQIzUCtMZD");
function postImage(img, msg, fn){
	...
	, function(res){
		if (res.error || !res.id)
			postImage(img,msg,fn);
		else
			fn(res);
	});
}
function getImage(id, fn){
	fb.api(id, 'get', 
	{        
	}
	, fn);
...
function Scraper(source){
	...
	this.history = 2; //articles to save in db
	this.success = [];
}
	...
	this.getList(function(err, new_urls){
		...
		async.mapSeries( 
					...
					client.hget(self.label, result.url, function(err,data){
						if(data){
							...
							if(data.current_image){
								...
								client.hset(self.label, result.url, JSON.stringify({article:article, current_image:result.images}), redis.print);
								return callback(err, null);						
							}
						}
						...
						//if image doesnt exist then postimage to fb
						postImage(result.images, result.images, function(res){
							console.log("Posting to fb");
							if(res.id){
								getImage(res.id, function(img){
									result.images = {};
									result.images.sizes = img.images;
									result.images.thumb = img.picture;
									//add to redis
									client.lpush('articles:' + self.label, JSON.stringify(result), redis.print);
									client.hset(self.label, result.url, JSON.stringify({article:article, current_image:result.images}), redis.print);
									callback(err, result);
			...
			function(err, data){
				...
				client.lrange('articles:' + self.label, self.history, self.history + 100,  function(err, data){
