...
Scraper.prototype.generateSequence = function(){
	...
	urls = urls.filter(function(elem){
		return elem.num > self.data.lastfetch;
	});
...
Scraper.prototype.maxScraped = function(){
	...
	for(var i=0; i<this.urls.length; i++){
		var url_num = this.urls[i].num;
		
		//remove elements in data.not_found which exists in this.urls
		if(this.exists(url_num, this.data.not_found)){
			continue;
		}
...
Scraper.prototype.exists = function(num, arr){
	//expects arr to be [{url:'', num:'', tries:''}]
	for(var i=0; i<arr.length; i++){
		if(arr[i].num === num){
			return true;
		}
...
MVYouth.prototype.scrapeTitle = function(dom){
	...
	if(title.indexOf("Website is currently unreachable") !== 0){
		return 404;
	}
	return title;
}
